...
class QLearner(object):
  ...
  def step_env(self):
    ...
    # if not initialized or epsilon greedy exploration
  ...
  def log_progress(self):
    ...
    if self.t % self.log_every_n_steps == 0 and self.model_initialized:
      ...
      file_name = 'data/ddqn_testing.csv'
      append_write = 'a' if os.path.exists(file_name) else 'w'

      with open(file_name, append_write) as f:
        writer=csv.writer(f)
        writer.writerow([self.mean_episode_reward, self.best_mean_episode_reward, self.t])
