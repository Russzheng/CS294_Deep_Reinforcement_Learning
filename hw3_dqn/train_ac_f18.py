...
def build_mlp(input_placeholder, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None):
    ...
    with tf.variable_scope(scope):
        hidden_layers[0] = tf.layers.dense(inputs=input_placeholder, units=size, activation=activation, 
            use_bias=True, kernel_initializer=tf.contrib.layers.xavier_initializer())
        for i in range(n_layers - 1):
            hidden_layers[i + 1] = tf.layers.dense(inputs=hidden_layers[i], units=size, activation=activation,
                use_bias=True, kernel_initializer=tf.contrib.layers.xavier_initializer())

        output_placeholder = tf.layers.dense(inputs=hidden_layers[-1], units=output_size, activation=output_activation,
            use_bias=True, kernel_initializer=tf.contrib.layers.xavier_initializer())
...
class Agent(object):
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        if self.discrete:
            # YOUR_HW2 CODE_HERE
            sy_logits_na = build_mlp(sy_ob_no, self.ac_dim, "policy_forward_pass", n_layers=self.n_layers, size=self.size)

            return sy_logits_na
        else:
            # YOUR_HW2 CODE_HERE
            sy_mean = build_mlp(sy_ob_no, self.ac_dim, "policy_forward_pass", n_layers=self.n_layers, size=self.size)
    ...
    def sample_action(self, policy_parameters):
        ...
        if self.discrete:
            ...
            # YOUR_HW2 CODE_HERE
            sy_sampled_ac = tf.distributions.Categorical(logits=sy_logits_na)
            sy_sampled_ac = tf.reshape(sy_sampled_ac.sample(), [-1])
        else:
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        if self.discrete:
            ...
            sy_mean, sy_logstd = policy_parameters
            # YOUR_HW2 CODE_HERE
            temp = (sy_ac_na - sy_mean) / tf.exp(sy_logstd)
            sy_logprob_n = - 0.5 * tf.reduce_sum(tf.square(temp), axis=1)
        return sy_logprob_n
    ...
    def build_computation_graph(self):
        ...
        self.critic_update_op = tf.train.AdamOptimizer(self.learning_rate * 1.25).minimize(self.critic_loss)
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        adv_n = re_n + self.gamma * v_sp1 - v_s
    ...
    def update_critic(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        for _ in range(self.num_target_updates):
            v_sp = self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no: next_ob_no})
            v_sp = (1 - terminal_n) * v_sp
            target = re_n + self.gamma * v_sp
