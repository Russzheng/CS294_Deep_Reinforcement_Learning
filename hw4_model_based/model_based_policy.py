...
class ModelBasedPolicy(object):
    ...
    def _dynamics_func(self, state, action, reuse):
        ...
        normalized_sa = tf.concat([utils.normalize(state, self._init_dataset.state_mean, self._init_dataset.state_std),
         utils.normalize(action, self._init_dataset.action_mean, self._init_dataset.action_std)], axis=1)

        delta_state_pred = utils.build_mlp(input_layer=normalized_sa, output_dim=self._state_dim, scope="dynamics_func",
            n_layers=self._nn_layers, reuse=reuse)

        next_state_pred = state + utils.unnormalize(delta_state_pred, self._init_dataset.delta_state_mean, self._init_dataset.delta_state_std)
    ...
    def _setup_training(self, state_ph, next_state_ph, next_state_pred):
        ...
        real_diff = utils.normalize(real_diff, self._init_dataset.delta_state_mean, self._init_dataset.delta_state_std)
        pred_diff = utils.normalize(pred_diff, self._init_dataset.delta_state_mean, self._init_dataset.delta_state_std)

        loss = tf.reduce_mean(tf.squared_difference(real_diff, pred_diff))
    ...
    def _setup_action_selection(self, state_ph):
        ...
        ### PROBLEM 2
        ### YOUR CODE HERE
        random_sequences = tf.random_uniform(shape=[self._num_random_action_selection, self._horizon, self._action_dim], 
            minval=self._action_space_low, maxval=self._action_space_high)

        costs = tf.zeros([self._num_random_action_selection,])
        state_ph = tf.squeeze(state_ph, [0])
        all_states = tf.concat([tf.expand_dims(state_ph, 0)] * self._num_random_action_selection, axis=0)

        for i in range(self._horizon):
            next_states = self._dynamics_func(all_states, random_sequences[:, i], True)
            costs += self._cost_fn(all_states, random_sequences[:, i], next_states)
            all_states = next_states

        # import sys; print(sys._getframe().f_code.co_name,sys._getframe().f_lineno)
        # from IPython import embed; embed()

        best_sequence = tf.argmin(costs)
        best_action = random_sequences[best_sequence, 0]
    ...
    def train_step(self, states, actions, next_states):
        ...
        ### MULTI-STEP ###
        # Not sure if I got this right honestly ;) #
        # losses = []
        # for _ in range(5):
            # import sys; print(sys._getframe().f_code.co_name,sys._getframe().f_lineno)
            # from IPython import embed; embed()
    ...
    def get_action(self, state):
        ...
        best_action = best_action.reshape(self._action_dim,)
