...
def build_rnn(x, h, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None, regularizer=None):
    ...
    GRU_cell = tf.contrib.rnn.GRUCell(output_size, activation=activation)
    ...
    def sample_trajectory(self, env, animate_this_episode, is_evaluation):
        ...
        while True:
            ...
            if ep_steps == 0:
                ...
                # YOUR CODE HERE
                a, r, b = np.zeros(self.ac_dim), np.zeros(self.reward_dim), np.zeros(self.terminal_dim)
                meta_obs[steps] = np.concatenate((ob, a, r, b))
                steps += 1
            ...
            if steps >= self.history:
                ...
                dummy = np.zeros((self.history - steps, self.meta_ob_dim))
                in_ = np.concatenate((dummy, meta_obs[:steps]), axis=0)
            ...
            # YOUR CODE HERE
            meta_obs[steps] = np.concatenate((ob, ac, np.array([rew]), np.array([done])))
            rewards.append(rew)
            steps += 1
